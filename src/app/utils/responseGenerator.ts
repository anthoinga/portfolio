...
export function generateFilterResponse(
  ...
  if (intent) {
    ...
    // No matches - use noResults template
    const template = getResponseTemplate(profile, 'noResults');
    ...
    // Showing all projects - use greeting template
    const template = getResponseTemplate(profile, 'greeting');
    response = template
      .replace('{count}', filter.totalMatches.toString())
      .replace('{range}', getProjectRange(allProjects));
  } else {
    // Has matches - use results template
    const template = getResponseTemplate(profile, 'results');
    const tech = extractQueryTech(filter.query);
